...
\label{sec:der_weg_zur_performance}
	...
	Dem Kunden muss Performance als visuell greifbares Erlebnis präsentiert werden. Dabei sind nicht nur Diagramme sondern auch Tools wie Webpagetest sehr nützlich. Damit können Vergleichsvideos erstellt werden. Denkbar wäre zum Beispiel ein direkter Vergleich mit den Konkurrenzseiten zu zeigen. Auch ein Vorher- / Nachervergleich eines erfolgreichen Projekts könnte dem Kunden präsentiert werden. Zum Launch kann dann ein Vergleich mit pre- und post-Performance aufgezeigt werden.\\
		...
		\label{ssub:performance_budget}
			...
			Ein Performance Budget bedeutet, dass ein Wert gesetzt wird, der von der Seite nicht überschritten werden darf. Dieser Wert kann ein ganz simpler sein, wie die Ladezeit der Seite oder aber eine komplexere Metrik aus verscheidenen Werten.\\
			Ein sehr simple Metrik währe es, den Speed Index als Budget zu setzen. Jede Seite sollte dann unter diesem festgelegten Wert bleiben. Oftmals ist es aber besser verschiedene Werte als Budget zu setzen. Eine Kombination aus: Anzahl Requests, maximale Seitengröße, maximales Gewicht der Bilder plus der Speed Index können eine sinnvolle und verständliche Basis bilden. Es können aber je nach Projekt auch anwendungsbezogene Metriken verwendet werden. So benutzt Twitter zum Beispiel die Metrik "`Time to first Tweet"'.
			...
			Aber auch Google stellt fest, dass bereits der Einfluss von 100 - 400 ms einen Messbaren einfluss auf die Anzahl an Suchanfragen (-0,2\% bis -0,6\%) pro Anwender hat.\autocite{google09} Während 0,4\% sich nach vergleichbar wenig anhört, so ist bei jährlich 2,2 Billionen Suchanfragen (2013) wohl klar, was das für Google an Mehreinnahmen durch Werbung und Klicks bedeuten kann.\autocite{statista14}
