...
\label{sec:entwicklung}
	...
	\label{sub:ausgangspunkt}
		...
		\label{ssub:render_blocking_javascript}
			Bereits unter Punkt \ref{ssub:rendering_pfad} ist das Blockierende Verhalten von Javascript und CSS angesprochen worden. Grundvoraussetzung für diesen Punkt ist, dass das Javascript der Webanwendung in ihre, für das Rendern kritische und für das Rendern unkritische Teile zerlegt wurde. Kleinere Scripts lassen sich auch direkt \texttt{Inline} in das HTML Dokument einfügen. Dadurch sind diese gleich mit dem ersten Request bereits im Dokument enthalten und müssen nicht erst per Request angefordert und heruntergeladen werden.\\
			...
			\begin{lstlisting}[captionpos=b, caption=Javascript nach \autocite{deferJsScript}, label=lst:deferJs]
			...
			Dieses Skript sagt im Grunde: Lade beide Dateien gleichzeitig herunter, beachte aber die Abhängigkeit (dependency) von deferJs.min.js gegenüber jquery. Ist defer.js früher heruntergeladen als jquery, so wartet das Skript und führt danach die Skripte in der richtigen Reihenfolge aus.\\

			Für die Webseite \url{http://andreaslorer.de} wurde sowohl die Methode "`<script defer>"', als auch das Skript \texttt{jQl} verwendet. "`<script defer src='critical.js'></script>"' wird dabei in den "`<head>"' bereich des HTML Dokments platziert, damit es möglichst früh erkannt wird und der Download bereits frühst möglich beginnen kann und das Skript aus Listing \ref{lst:jQl} befindet sich vor dem "`</body>"'-Tag.
